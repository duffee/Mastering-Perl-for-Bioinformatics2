...
# just add the new attributes, the other 4 are inherited
enum 'Format', [qw(text png jpg)];

has graphic     => (is => 'rw', isa => 'Str',    default => '');
has graphictype => (is => 'rw', isa => 'Format', default => 'text');
...
sub _drawmap_text {
    ...
    my @annotation = ();
    push @annotation, _initialize_annotation_text($self->sequence);
    ...
    my @output = _formatmaptext(50, $self->sequence, @annotation);
...
sub _add_annotation_text {
    ...
    foreach my $location (@pos) {
        ...
        for (my $i = 0; $i < @$array; ++$i) {
            ...
            if (substr($array->[$i], $location - 1, length($enz)) eq
                (q{ } x length($enz)))
            {
