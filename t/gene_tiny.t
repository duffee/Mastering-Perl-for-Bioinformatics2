...
like(
    ...
    qr/Usage: Gene::Tiny::name\(self\) at/,
...
is( Gene::Tiny->get_count(), 2, 'Two objects created' );
...
is( Gene::Tiny->get_count(), 3, 'Three objects created' );

# Test object creation when missing required arguments
my %option_args = (chromosome => '23', pdbref => 'pdf9999.ref');

like(
    dies { Gene::Tiny->new( organism => 'Homo sapiens', %option_args ) },
    qr/Missing required arguments: name/,
    'Dies when name argument missing'
);

is( Gene::Tiny->get_count(), 3, 'Bug fixed, still have 3 Genes' );

like(
    dies { Gene::Tiny->new( name => 'Aging', %option_args ) },
    qr/Missing required arguments: organism/,
